...
import CHECKLIST_FIELDS from '../assets/checklist_field';
...
const GeneratorChecklistDetailView = ({ data: initialData, onEdit, onClose }) => {
    ...
    <div className="min-h-screen bg-gray-900 text-white p-4 md:p-8 flex justify-center">
      <div className="w-full max-w-4xl relative pb-24">
        <div className="flex justify-between items-center mb-8 border-b border-gray-800 pb-4">
          <div>
            <h2 className="text-2xl font-black text-[#22c55e] uppercase">{data.Unit || 'Unit'} - Checklist Record</h2>
          </div>
          <button onClick={onClose} className="text-gray-500 hover:text-white text-4xl">&times;</button>
        </div>

        <div className="space-y-3">
          {CHECKLIST_FIELDS.map((field, index) => {
...
export default GeneratorChecklistDetailView;